...
class AdminCtl {
    async create(ctx) {
        ...
        if( repeateAdmin ) { ctx.throw(409, '该邮箱已经注册')}
    ...
    async login(ctx) {
        ...
        if(!admin) { ctx.throw(401, '用户名或者密码不正确')}
        const correct = bcrypt.compareSync(password, admin.password)
        if(!correct) { ctx.throw(401, '用户名或者密码错误') }
        const token = jsonwebtoken.sign({ email, scope: AUTH_ADMIN, id: admin.id }, secret, { expiresIn: '1d'})
