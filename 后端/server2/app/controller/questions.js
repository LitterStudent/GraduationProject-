...
class QuestionsCtl {
    ...
    async findById(ctx) {
        ...
        if(!question || question.status != 1) { ctx.throw(404, '问题不存在')}
        const topic_id = question.topic_id
        const topoic = await Topic.findByPk(topic_id)
        ctx.body = { question, topoic}
    }
    async create(ctx) {
        ctx.verifyParams({
    ...
    async checkQuestioner(ctx, next) {
        ...
        if(question.created_user != ctx.auth.id) { ctx.throw(403, '没有权限') }
        await next();
    }
    async followQuestion(ctx) {
        const user_id = ctx.auth.id
        const question_id = ctx.params.id
        const followQuestion = await FollowQuestion.findOne({ where: { user_id, question_id }})
        if (!followQuestion) {
    ...
    async listFollowingQuestions(ctx) {
        ...
        const followQuestionList = await FollowQuestion.findAll({ where: { user_id }})
        const questionIds = followQuestionList.map(item => item.question_id)
        const questionList = await Question.findAll({ where: {
            id: {
                [Op.in]: questionIds
            }
        }})
